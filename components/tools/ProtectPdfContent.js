...
import { protectPdfUri } from '@/lib/pdf/protect';
import { uint8ArrayToBase64 } from '@/lib/utils/file';

const PDF_MIME = 'application/pdf';

export default function ProtectPdfContent() {
  ...
  const protect = async () => {
    ...
    setError(null);
    try {
      const bytes = await protectPdfUri(file.uri, password.trim());
      const filename = `kitbase-protected-${Date.now()}.pdf`;
      ...
      setError(e.message ?? 'Protect failed');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <View style={[styles.scrollContent, { paddingBottom: Math.max(24, insets.bottom) }]}>
      <ThemedText style={styles.description}>
        Encrypt a PDF with a password. Readers will need the password to open it.
      </ThemedText>
      ...
      >
        {loading ? (
          ...
          <>
            <KitbaseIcon name="Lock" size={20} color="#fff" />
